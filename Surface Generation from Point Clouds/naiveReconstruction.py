...
def naiveReconstruction(points, normals, X, Y, Z):
    ...
    # a list of query points Q = {(x,y,z)}.
    Q = np.stack([X.reshape(-1), Y.reshape(-1), Z.reshape(-1)], axis=1)  #p

    # Use a KD-tree to find, for each grid point p, the closest input point p_j.
    tree = KDTree(points)
    _, idx = tree.query(Q, k=2)  
    nearest_idx = idx[:, 0]

    nearest_points = points[nearest_idx]  # p_j
    nearest_normals = normals[nearest_idx]  # n_j
    
    # Compute the signed distance from each grid point to the tangent plane of its nearest point using the formula: 
    # sdf(p) = n_j . (p - p_j)
    sdf = np.sum(nearest_normals * (Q - nearest_points), axis=1)

    IF = sdf.reshape(X.shape)  
