...
def train(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        ##########################################################       
        # clamped L1 loss: | clamp(pred, sigma) - clamp(gt, sigma) |
        pred_sdf_tensor = torch.clamp(pred_sdf_tensor, -args.clamping_distance, args.clamping_distance)
        gt_sdf_tensor = torch.clamp(gt_sdf_tensor, -args.clamping_distance, args.clamping_distance)
...
def val(dataset, model, optimizer, args):
    ...
    for i in range(num_batch):
        ...
        # clamped L1 loss in validation mode 
        with torch.no_grad():
            gt_sdf_tensor = data['gt_sdf'].to(device)
